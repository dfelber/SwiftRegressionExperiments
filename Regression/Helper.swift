...
func ⋅ (lhs: Matrix, rhs: Matrix) -> Double {
    return dot(lhs, rhs)
}


/// Dot product of two matrices
func dot(lhs: Matrix, _ rhs: Matrix) -> Double {
    var result = 0.0
...
/// Matrix multiplication
func multiply(lhs: Matrix, _ rhs: Matrix) -> Matrix {
    var result = [Double](count : Int(lhs.rows * rhs.columns), repeatedValue : 0.0)
...
func concat(v1: [Double], _ v2: [Double]) -> [Double] {
    var values = [Double](count: v1.count + v2.count, repeatedValue: 0)
...
func solve(A: [Double], _ B: [Double], TRANS: String = "T") -> [Double] {
